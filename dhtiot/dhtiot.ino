...
void buzzerOn() {
  ...
  server.send(200, "text/html", "<h1>Hai Engineer</h1><h2>Buzzer is on</h2><button onclick='location.href=\"/buzzeroff\"' type='button'>Buzzer Off</button><button onclick='location.href=\"/sensorcheck\"' type='button'>Check Sensor Value</button>");
}

void buzzerOff() {
  digitalWrite(BUZZER, LOW); // turn the LED on (HIGH is the voltage level)
  server.send(200, "text/html", "<h1>Hai Engineer</h1><h2>Buzzer is off</h2><button onclick='location.href=\"/buzzeron\"' type='button'>Buzzer On</button><button onclick='location.href=\"/sensorcheck\"' type='button'>Check Sensor Value</button>");
}
...
String sensorRead() {
  ...
  String dhtData = "Current humidity = " + String(h, 0) + "% temperature = " + String(t, 0) + "C\n";
  return dhtData;
}

void sensorCheck() {
  String sensorData = sensorRead();
  server.send(200, "text/html", "<h1>Hai Engineer</h1><h2>See the sensor value</h2> " + sensorData + "<br><button onclick='location.href=\"/buzzeron\"' type='button'>Buzzer On</button><button onclick='location.href=\"/buzzeroff\"' type='button'>Buzzer Off</button><button onclick='location.href=\"/sensorcheck\"' type='button'>Check Sensor Value</button>");
}
